...
export default function RoomGallery({ images, emptyLabel }: { images: Img[]; emptyLabel?: string }) {
  const [index, setIndex] = useState<number | null>(null);
  if (images.length === 0) {
    return (
      <div className="flex h-52 w-full items-center justify-center rounded-lg bg-[var(--color-surface-secondary)] text-sm text-text-muted">
        {emptyLabel ?? ''}
      </div>
    );
  }
