...
export function validateContent(meta: BuildingMeta[], disk: DiskBuilding[]): ValidationResult {
  ...
  for (const b of meta) {
    ...
    if (!d.hasCover) errors.push(`Building "${b.folder}" is missing a cover image (cover.jpg/jpeg/png/webp)`);
    ...
    for (const dr of d.rooms) {
      ...
      if (!dr.hasCover) errors.push(`Room "${dr.folder}" in "${b.folder}" is missing a cover image (cover.jpg/jpeg/png/webp)`);
    }
    for (const r of b.rooms) {
